...
has 't' => (
  ...
  isa => sub {
    ...
    die "$_[0] is not a Net::Telnet Object!" unless ref $_[0] eq 'Net::Telnet';
  },
...
sub currentTrack
{
  ...
  my $album =
    $self->pick_field( 4,
    $self->runcommand( "$playerId playlist album $currentTrackNumber ?" ) );
  my $artist =
    $self->pick_field( 4,
    $self->runcommand( "$playerId playlist artist $currentTrackNumber ?" ) );
  my $title =
    $self->pick_field( 4,
    $self->runcommand( "$playerId playlist title $currentTrackNumber ?" ) );
...
sub playlist
{
  ...
  for my $i ( 0 .. $playlistLength - 1 )
  {
    ...
    my $artist =
      $self->pick_field( 4,
      $self->runcommand( "$playerId playlist artist $i ?" ) );
    my $title =
      $self->pick_field( 4,
      $self->runcommand( "$playerId playlist title $i ?" ) );

    my $iscurrent = ( $i == $currentTrackNumber ) ? '>' : ' ';
    say sprintf "%s %3d - '%s' from %s by %s", $iscurrent, $i, $title, $album,
      $artist;
